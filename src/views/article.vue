<template>
  <div class="articleWrap" v-if="article.article_info">
    ...
    <div class="article">
      <div class="contentWrap">
            ...
            <div class="public">
              <div class="top">
                ...
                <img
                  :src="
                    require('@/assets/lv-' +
                      article.author_user_info.level +
                      '.png')
                  "
                  alt=""
                />
              ...
              <div class="bottom">
                {{ formateDate }} · 阅读 {{ article.article_info.view_count }}
              </div>
            </div>
          </div>
          <el-button
        ...
        <div class="markWrap markdown-body">
          <VueMarkdown
            :source="article.article_info.mark_content"
            class="markContent markdown-body"
            ref="markRef"
          ></VueMarkdown>
        </div>
        <div class="end">
          <span
            >分类:<el-button
              ...
              class="classify"
              size="small"
              @click="goto(article.category.category_id)"
              >{{ article.category.category_name }}</el-button
            ></span
          >
          <span
            >标签:<el-button
      ...
      <div class="comment">
        ...
        <div class="inputWrap">
          <img
            src="https://p9-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/84d3d27a3c364a6b997dfa25d21351d8~tplv-k3u1fbpfcp-no-mark:240:240:240:160.awebp?"
            alt=""
            class="commentAvator"
          />
          <el-input
    ...
    <aside class="asside">
      <div class="authorDesc">
        ...
        <div class="authorBottom">
          <div class="like">
            ...
            获得点赞<span>{{
              article.author_user_info.got_digg_count.toLocaleString()
            }}</span>
          </div>
          <div class="see">
            <svg
            ...
            文章被阅读<span>{{
              article.author_user_info.got_view_count.toLocaleString()
            }}</span>
          </div>
        </div>
      </div>
      ...
      <Advertise
        imgSrc="https://p1-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/7d17b46ae3a64f7799ea474d0c5762c3~tplv-k3u1fbpfcp-no-mark:480:400:0:0.awebp?"
      />
      <div class="relateArticle">
        ...
        <div class="articleList">
          <div
            ...
            :key="item.article_id"
            @click="seeArticle(item)"
          >
            <div class="title">{{ item.article_info.title }}</div>
            <span
              >{{ item.article_info.digg_count }}点赞 ·
              {{ item.article_info.comment_count }}评论</span
            >
          </div>
        </div>
      </div>
      ...
      setTimeout(() => {
        this.$nextTick(() => {
          ...
          childList = Array.from(childList).filter((el) => {
            return (
              arr.indexOf(el.nodeName) > -1 &&
              el.innerText.indexOf("theme") == -1
            );
          });
